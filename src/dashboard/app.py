...
with col_vs:
    st.markdown("<br><br><div style='text-align:center; font-size:24px; font-weight:bold; color:#ffd700'>VS</div>", unsafe_allow_html=True)
with col_away:
    away_list = [t for t in teams_list if t != home]
    away = st.selectbox("✈️ Away Team", away_list, key="away_sel")
...
if predict_clicked:
    if home == away:
        ...
        with st.spinner(f"Calculating probabilities for {home} vs {away}..."):
            try:
                # Call the API match predictor
                resp = requests.post(
                ...
                # Verdict
                if hw > dr and hw > aw:
                    verdict = f"🏠 **{home}** are favoured to win at home ({hw:.1f}%)"
                    color   = "#4CAF50"
                elif aw > dr and aw > hw:
                    verdict = f"✈️ **{away}** are favoured to win away ({aw:.1f}%)"
                    color   = "#2196F3"
                else:
                    verdict = f"🤝 A draw is the most likely outcome ({dr:.1f}%)"
                    color   = "#9E9E9E"
